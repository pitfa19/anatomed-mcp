...
## Sibling: the Obsidian plugin (`anatomed-obsidian`)

The same viewer also ships as a **standalone Obsidian community plugin** in its own repo,
**[anatomed-obsidian](https://github.com/pitfa19/anatomed-obsidian)** (not in this tree). It is
self-contained but **vendors this project's core** — copies of
`src/{region,neighbors,shared,catalog,vendor/*}.ts`, `widget/{RegionViewer.tsx,lib/three-helpers.ts,styles.css}`,
and `assets/parts-catalog.json`. These are COPIES: if you change the resolver or viewer core here,
port the change over (same "keep vendored copies in sync" discipline as the anatomed-web vendoring).

- Renders `` ```anatomed `` code blocks via `registerMarkdownCodeBlockProcessor` + a React root,
  reusing `buildRegion` and `RegionViewer` unchanged. `isDesktopOnly: true` (the vendored
  `catalog.ts`/`neighbors.ts` do top-level `node:fs`; mobile is a TODO in that repo).
- **Fetches GLBs and `parts-neighbors.json` at runtime** from the shared Supabase asset host
  (`${assetBase}/…`) — so `npm run upload:assets` here now also uploads `parts-neighbors.json`
  (the plugin's `related`/`regional` context data). The plugin bundles only `parts-catalog.json`.
- **Publishing:** built with its own esbuild config → GitHub Release (tag == `manifest.version`,
  assets `main.js` + `manifest.json` + `styles.css`); submitted once at community.obsidian.md; the
  directory bot reads `manifest.json` at the repo-root HEAD + the release assets. `versions.json`
  maps version → minAppVersion; the demo video is a release asset of that repo.
...
- **Licensing = unified CC BY-SA 4.0.** The **entire project** — code AND the 3D models + derived data (GLBs, `parts-catalog.json`, `parts-neighbors.json`) — is **CC BY-SA 4.0** (`LICENSE` holds the full legal code, © Fabijan Pitlović). Relicensed 2026-07-22 from the old dual MIT-code / CC-assets split (LICENSE-ASSETS deleted) so the whole work is one copyleft-with-attribution license matching **Z-Anatomy** (CC BY-SA 4.0) → **BodyParts3D/DBCLS** (CC BY-SA 2.1 JP). Attribution + share-alike required; indicate changes. Credits live in `NOTICE`, README "Licensing"/"Credits", and an in-widget footer (`.am-legend-credit` in `widget/RegionViewer.tsx`). `CITATION.cff` points to the STC 2026 paper. Reason for the switch: the Z-Anatomy author objected to MIT-on-code atop a CC BY-SA model and wanted copyleft + guaranteed attribution; unifying to CC BY-SA moots the "is the plugin a derivative of the model" argument. **This copyleft now follows the vendored viewer core into `anatomed-obsidian` (also relicensed) and would follow it into `anatomed-web` too.**
