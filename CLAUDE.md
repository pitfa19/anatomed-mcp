...
- **Licensing = dual.** Code is **MIT** (`LICENSE`, © Fabijan Pitlović). The 3D models + derived data (GLBs, `parts-catalog.json`, `parts-neighbors.json`) are **CC BY-SA 4.0** (`LICENSE-ASSETS`), because they derive from **Z-Anatomy** (CC BY-SA 4.0) → **BodyParts3D/DBCLS** (CC BY-SA 2.1 JP). Attribution is required + share-alike: don't relicense the models as MIT. Credits live in `NOTICE`, README "Credits & data sources", and an in-widget footer (`.am-legend-credit` in `widget/RegionViewer.tsx`). `CITATION.cff` points to the STC 2026 paper.
